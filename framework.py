...
# MSR and R^2 on train set
train_predictions = np.dot(train_with_bias, params)
train_mse = np.mean((y_train - train_predictions) ** 2)
train_r2 = r_2(y_train, train_predictions)
...
print(f"Test MSE: {test_mse:.4f}, Test R²: {r2_test:.4f}")
...
plt.scatter(range(len(y_test)), y_test, color='pink', label='True Values', alpha=0.6)
plt.scatter(range(len(predictions)), predictions, color='purple', label='Predictions', alpha=0.6)
